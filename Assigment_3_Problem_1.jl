...
# ╔═╡ 0b0b2854-7f85-47eb-b05d-dff3eb7c96e8
using CSV

# ╔═╡ 94eda5e2-16c5-469d-94ee-e756199ef82d
using DataFrames

# ╔═╡ ca1a3c86-696c-4ee5-88b6-7756a9ba9d72
using Flux

# ╔═╡ 94bdebae-a1d1-44bb-94e1-a2f3c98b8ba9
using Flux: onehotbatch, onecold, logitcrossentropy

# ╔═╡ 87d8db76-2b09-4847-bd02-3a90de3ee028
using WordTokenizers

# ╔═╡ f6040660-0a59-415b-a7f7-98bb92d42739
using Random

# ╔═╡ 4fd18988-ee2d-4cb0-9a21-d22219ca028e
Random.seed!(123)

# ╔═╡ 45db775a-da8b-4822-8d75-14de7df96431
cd("/Users/eandreas/Documents/School/NUST 2025/Semester 1/TAI911S1/Assignment 3/")

# ╔═╡ 1e40f8ff-9664-4bac-95ae-ffad21d301d2
train_df = CSV.read("./dataset/medical_tc_train.csv", DataFrame)

# ╔═╡ e928e080-035c-4503-8503-1a89d8ab002c
test_df = CSV.read("./dataset/medical_tc_test.csv", DataFrame)

# ╔═╡ 69c38460-0f20-42c4-bc18-eed78d8246da
labels_df = CSV.read("./dataset/medical_tc_labels.csv", DataFrame)

# ╔═╡ da95801d-f2ba-4c75-81d4-2aa61316667b
begin
	train_df1 = leftjoin(train_df, labels_df, on =:condition_label => :condition_label)
	test_df1 = leftjoin(test_df, labels_df, on = :condition_label => :condition_label)
		
end

# ╔═╡ 1af11d8d-ff4f-4389-b6bf-6eb826ec3b24
all_text = vcat(train_df1.medical_abstract, test_df1.medical_abstract)

# ╔═╡ e722691e-40b7-489b-b16c-c53bcce3c852
function tokenize_text(text)
    tokens = split(lowercase(text), r"[^a-z0-9]+") # Simple alphanumeric tokenizer
    filter!(t -> !isempty(t), tokens) # Remove empty tokens
    return tokens
end

# ╔═╡ 0c9c3997-afa6-4310-ad21-93aaf6b8f1d1
begin
	tokenized_train = tokenize_text.(train_df1.medical_abstract)
	tokenized_test = tokenize_text.(test_df1.medical_abstract)
end

# ╔═╡ 3c3fe62f-d5d7-488e-bb5f-f61e7b64758a
begin
    vocabulary = Dict{String, Int}()
    idx = 1 # Initialize idx here

    for tokens in tokenized_train
        for token in tokens
            if !haskey(vocabulary, token)
    ...
    vocabulary["<unk>"] = idx # Unknown token
    unk_idx = idx
    idx += 1
    vocabulary["<pad>"] = idx # Padding token
    pad_idx = idx

    vocab_size = length(vocabulary) # Now includes <unk> and <pad>
end 

# ╔═╡ 7a083737-f1f3-4749-846e-bce025a9ea93
begin
	println("Vocabulary size: $(vocab_size)")
	println("Unknown token index: $(unk_idx)")
	println("Padding token index: $(pad_idx)")
end

# ╔═╡ f1b0fad3-944c-4c47-a961-14c6a9d38a0a
begin
	max_len = 50 # Define a maximum sequence length, adjust as needed
	
	function numericalize_and_pad(tokenized_texts, vocab, max_len, unk_idx, pad_idx)
	    numerical_sequences = []
	    for tokens in tokenized_texts
	        num_seq = [get(vocab, token, unk_idx) for token in tokens]
	        # Pad or truncate sequences
	        if length(num_seq) > max_len
	            num_seq = num_seq[1:max_len]
	        elseif length(num_seq) < max_len
	            append!(num_seq, fill(pad_idx, max_len - length(num_seq))) # Pad with pad_idx
	        end
	        push!(numerical_sequences, num_seq)
	    end
	    # Convert to a matrix where each column is a sequence
	    return hcat(numerical_sequences...)
	end
end

# ╔═╡ c0b0da9c-3a0d-4fb5-955a-19a682bfef22
begin
	# Numericalize and pad data
	X_train = numericalize_and_pad(tokenized_train, vocabulary, max_len, unk_idx, pad_idx)
	X_test = numericalize_and_pad(tokenized_test, vocabulary, max_len, unk_idx, pad_idx)

	println("X_train dimensions: $(size(X_train))")
	println("X_test dimensions: $(size(X_test))")

	# Prepare labels (one-hot encoding)
	unique_labels = sort(unique(train_df1.condition_label))
	...
	Y_train_raw = [label_map[label] for label in train_df1.condition_label]
	Y_test_raw = [label_map[label] for label in test_df1.condition_label]

	Y_train = onehotbatch(Y_train_raw, 1:num_classes)
	Y_test = onehotbatch(Y_test_raw, 1:num_classes)

	println("Y_train dimensions: $(size(Y_train))")
	println("Y_test dimensions: $(size(Y_test))")

	# Ensure data types are Float32 for Flux (removed Flux.param)
...
# ╔═╡ 382e73d2-f306-486c-9a1a-cf6c1da29a63
begin
	println("3. Building the LSTM model...")
...
# ╔═╡ 320bba83-cb2a-4458-8482-84cfea936b5c
begin
...
# ╔═╡ 09d47619-3020-4ff7-9a56-55adbc1f0811
begin
	println("4. Training the model...")
	...
	for epoch in 1:epochs
	    ...
	    for (x_batch, y_batch) in train_data
	        x_batch_int = Int64.(x_batch)
	
	        current_loss_value = loss(model, x_batch_int, y_batch)

	        gs_tuple = gradient((m) -> loss(m, x_batch_int, y_batch), model)

	        Flux.update!(optimizer_state, model, gs_tuple[1])
...
# ╔═╡ dea108ca-1a60-41f8-9c5a-84ed0c836dc0
# Evaluate on test set
function accuracy(model, X, Y)
    y_pred = model(Int64.(X))

    return sum(onecold(y_pred) .== onecold(Y)) / size(Y, 2)
end

# ╔═╡ 63010d79-47b2-49ea-8ac8-79be458a5420
begin
...
manifest_format = "2.0"
project_hash = "f79f548448ff824fa7a8e2292d209ad8af2b33d0"
